...
During the search for related work there was no such comparison of solving-methods for 3x3x3 cube found. The information found about solving-methods for 3x3x3 cubes was tutorials of how to use them.
...
\chapter{Method}
For this thesis the two major methods used where implementation of methods and analyzis of the datarepresentation.
...
\section{Cube representation}
 The cube will be represented with a side-representation, meaning that an object for each side of the cube will hold nine colours. This will conduce to that the operations only have to move the colours to the correct side and location when an operation is performed.  
With this representation will there be no relation between positions in different sides unlike the physical cube, where for example an edge cubie has two colors. \\\\
For instance if operation F is performed on the cube all of the colours on the front-side of the cube except the center-colour (position 5) will move two steps clockwise (see fig \ref{fig_17} for positions). The colours on the top,right,bot and left side that are adjecent to the front side will all move three steps clockwise. Making the three colours of the right side that are adjacent to the front side move to the bot side, the three colours on the bot side moves to the left side and so on.
...
Fig \ref{fig_20} represents the avarage use of operations for both methods over all executions with the x-axis as the avarage number of times the operations where used. \\The data for both fig \ref{fig_19} and fig \ref{fig_20} can be seen in Appendix B.\\\\ 
...
\begin{figure}[ht!]
	...
	\caption{Avarage use of operations}
	\label{fig_20}
\end{figure}
...
\section{Difficulty}
Our first idea was to represent the cube by 26 cubies in a layer-by-layer system, but this was problematic due to which colour-representation of the cubies should face which side when different operations where performed. A pro for this first idea was that the colors of a cubie had a relation which kept the colours to the correct cubie through all the operations, the only thing keeping the colours correct now is the operations implemented. But a pro with the representation now is that the operations only need to move colours around according to the different operations (as described in cube representation) and does not need to rotate individual cubies to get the colour in the right position. \\\\
Translating the methods for physical sovling of the cube to code was problematic with the intuative steps forsolving by hand, this made it difficult implement in a good way.
