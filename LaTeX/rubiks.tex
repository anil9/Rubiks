...
\section{Method implementation}
	...
	\paragraph{Complexity} With methods implemented as desired with the same type of data structures as described above . The steps of both methods will use the same general idea with the same datastructures in the implementation [Algorithm 2], this will result in faciliation during the comparison with no big differences in implementations. Both methods runs in linear time even with while-loops that break for example when a step is done. These loops has a constant maximum number of iterations. This is because each step will focus on a finite part of the cube, which results in a finite amount of permutations to try when a permutation not matches a desired state.
	\RestyleAlgo{boxed}
	\begin{algorithm}[H]
...
>>>>>>> 4177ea8b983cbf05a6081eb8afd958d06fb1c899
