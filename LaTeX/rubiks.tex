...
The daisy algorithm refers to solve the white cross by first make a cross with white edges and a yellow center and then turn the white edges over to the opposite side, completing the white cross in the bottom \cite{Shellie}.
...
\chapter{Method}
For this thesis the two methods used were implementation of algorithms and analyzis of the data representation. An implementation was motivated considering that implementations allowed analyzation and collection of data from multiple algorithm runs.
...
\section{Implementation}
	...
	\raggedbottom
	\paragraph{Complexity} The steps of both algorithms used the same general idea with the same data-structures in the implementation (see [Algorithm 2]). Using the same data-structure simplifies the comparison of the algorithms due to no big differences in implementations. Both algorithms runs in linear time: the loops that break when a step is complete has a constant maximum number of iterations, because each step will focus on a finite part of the cube, resulting in a finite amount of permutations to try.
	\RestyleAlgo{boxed}
	\begin{algorithm}[H]
...
This shows that over ten thousand scrambled cubes the Lbl with daisy algorithm won 71\% of the time, they got equal amount of moves 1\% of the time and dedmore algorithm had the fewest 28\% of the time.\\\\
...
The data in fig \ref{fig_20} and Appendix B shows that Dedmore used both E, Ei and M, Mi more than Lbl with daisy. 		     
...
According to the assertion built on the authors own experience that some operations are more time consuming than others; Layer-by-layer using daisy algorithm is more suitable for speedcubing than Dedmore. This conclusion is based on the statistics of operation-usage, showing that Dedmore used these time-consuming operations seven times more frequently than Lbl with daisy algorithm.
