...
An operation is a movement of a layer on the cube and is communicated by notations in the Rubik's community. The notation describes the different move operations. This thesis uses the notation used by the World Cube Association (WCA) \cite{WCA1} explained below:\\
...
The authors own experience of the operations indicates that the operations E and M are most time-consuming because they both work with the middle-layer (horizontally and vertically). The data in fig \ref{fig_20} and Appendix B shows that Dedmore used both E,Ei and M, Mi more than Lbl with daisy. 		     
...
According to the assertion built on the authors own experince Layer-by-layer using daisy method is more suitable for speedcubing than Dedmore. This conclusion is made on the statistics of operation-usage that shows that Lbl using daisy method did not use operation M and Mi at all while Dedmore used 6.1943 each in average. Operations E and Ei had an average use of 0.1197 and 4.5945 towards Dedmores usage for the same methods with 6.0673 and 13.3863.
